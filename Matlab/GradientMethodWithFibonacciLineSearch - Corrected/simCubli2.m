function [ Ym ] = simCubli2( J_f, B_f, saveJ_f, saveB_f )

    assignin('base', 'J_f', J_f);
    assignin('base', 'B_f', B_f);

%      if exist('cubliParameterEstimation2', 'file')
%          simStatus = get_param('cubliParameterEstimation2','SimulationStatus');
...
%      end
    ...
    try
        warning('off', 'all');
        sim('cubliParameterEstimation2.slx');
        warning('on', 'all');
    catch errormsg
